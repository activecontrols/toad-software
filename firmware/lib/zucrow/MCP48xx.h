...
template <uint8_t BITS_RES> class MCP48xx {
...
public:
  enum Channel : uint8_t { A = 0u, B = 1u };

  enum Gain : uint8_t { High = 0u, Low = 1u };

  explicit MCP48xx(SPIClass &spi, pin_size_t _cs);
...
template <uint8_t BITS_RES> MCP48xx<BITS_RES>::MCP48xx(SPIClass &spi, pin_size_t _cs) : spi(spi), cs(_cs) {
  /* Setting channel bits*/
  command[Channel::A] = command[Channel::A] | (0u << 15u);
  command[Channel::B] = command[Channel::B] | (1u << 15u);
}

template <uint8_t BITS_RES> MCP48xx<BITS_RES>::~MCP48xx() {}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setVoltage(uint16_t value, Channel channel) {
...
template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setVoltageA(uint16_t value) {
  setVoltage(value, Channel::A);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setVoltageB(uint16_t value) {
  setVoltage(value, Channel::B);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::shutdownChannel(Channel channel) {
...
template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::shutdownChannelA() {
  shutdownChannel(Channel::A);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::shutdownChannelB() {
  shutdownChannel(Channel::B);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::turnOnChannel(Channel channel) {
...
template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::turnOnChannelA() {
  turnOnChannel(Channel::A);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::turnOnChannelB() {
  turnOnChannel(Channel::B);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setGain(Gain gain, Channel channel) {
  command[channel] = command[channel] | (gain << 13u);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setGainA(Gain gain) {
  setGain(gain, Channel::A);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::setGainB(Gain gain) {
  setGain(gain, Channel::B);
}

template <uint8_t BITS_RES> void MCP48xx<BITS_RES>::updateDAC() {
