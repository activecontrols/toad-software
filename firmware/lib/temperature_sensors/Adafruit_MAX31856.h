...
// Modifed by Robert Nies to remove dependency on Adafruit_SPIDevice
// We only use Hardware SPI on Toad
...
typedef enum { MAX31856_NOISE_FILTER_50HZ, MAX31856_NOISE_FILTER_60HZ } max31856_noise_filter_t;
...
public:
  Adafruit_MAX31856(SPIClass &spi_bus, unsigned int cs_pin, max31856_thermocoupletype_t tc_type)
      : spi_bus(spi_bus), cs_pin(cs_pin), tc_type(tc_type) {}
